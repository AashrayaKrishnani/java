...
// !!JAVA NIO!!
// JavaNIO package differs from JavaIO as such that in JavaIO the threads are blocking threads,
// While the threads in JavaNIO are non-blocking and optimized :D
...
public class Locations implements Map<Integer, Location> {
    ...
    // We use FileSystems.getDefault().getPath(), because it allows us to write the RELATIVE path
    //  of the file we want to refer to.
    // It appends the relative file path passed to getPath(), and appends it to the Absolute path of
    // the current working directory :D

    static Path locFilePath = FileSystems.getDefault().getPath("/code/JAVA/JavaIO/JavaNIO/locFile.txt");
    static Path exitsFilePath = FileSystems.getDefault().getPath("/code/JAVA/JavaIO/JavaNIO/exitsFile.txt");
    ...
    public Locations(String locFilePath, String exitsFilePath, boolean saveData, boolean saveInBinary, boolean readInBinary)  {
        Locations.locFilePath = FileSystems.getDefault().getPath(locFilePath);
        Locations.exitsFilePath = FileSystems.getDefault().getPath(exitsFilePath);
...
//         Initializing by reading data from TEXT files using Scanner and FileReader.
        if(!readInBinary)
        {
            try(Scanner locFile = new Scanner(Files.newBufferedReader(locFilePath)) ;
                Scanner exitsFile = new Scanner(Files.newBufferedReader(exitsFilePath)))
            ...
            String temp[] = locFilePath.toUri().getPath().split("[.]");        // Regex ke pachde XD
            Locations.locFilePath = FileSystems.getDefault().getPath((temp[0] + ".bin"));
            temp = Locations.exitsFilePath.toUri().getPath().split("[.]");
            Locations.exitsFilePath = FileSystems.getDefault().getPath((temp[0] + ".bin"));;


            // Reading data in Binary

            try(DataInputStream locFile = new DataInputStream(Files.newInputStream(locFilePath)))
            {                               // BufferedReader uses an 8Kb buffer (default), hence is much faster!!!
                // Reading locations
            ...
            try(DataInputStream exitsFile = new DataInputStream(Files.newInputStream(exitsFilePath))){
        ...
        if(saveData && !saveInBinary){
            ...
            try (BufferedWriter locFile = Files.newBufferedWriter(locFilePath);
                 BufferedWriter exitsFile = Files.newBufferedWriter(exitsFilePath)) {
        ...
        if(saveData && saveInBinary){
            ...
            try (DataOutputStream locFile = new DataOutputStream(Files.newOutputStream(locFilePath));
                 DataOutputStream exitsFile = new DataOutputStream(Files.newOutputStream(locFilePath))) {
